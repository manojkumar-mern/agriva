(function($) {
  ...
  // Image Trail ANimatoin
    ...
    /* ================================
       Universal Form & Web3Forms Email Submission
    ================================ */
    var WEB3FORMS_ACCESS_KEY = "287780f3-45a6-4f99-a5c1-5d1b4c795c84";

    $(document).on('submit', 'form', function(e) {
        var $form = $(this);

        // Prevent top of page reload / # redirect
        e.preventDefault();

        // Check if form is contact form
        if ($form.attr('id') === 'contact-form' || $form.attr('id') === 'contact-form2' || $form.find('textarea').length > 0) {
            var $msgDiv = $form.find('.form-message');
            if ($msgDiv.length === 0) {
                $msgDiv = $('<div class="form-message mt-3"></div>').appendTo($form);
            }

            var formData = new FormData($form[0]);
            formData.append('access_key', WEB3FORMS_ACCESS_KEY);

            $msgDiv.removeClass('error').addClass('success').css({
                'color': '#11A21B',
                'font-weight': '600',
                'padding': '10px 15px',
                'background': 'rgba(17, 162, 27, 0.1)',
                'border-radius': '6px',
                'border': '1px solid rgba(17, 162, 27, 0.3)'
            }).text('Sending message...').show();
            ...
            .then(function(data) {
                if (data.success) {
                    $msgDiv.css({'color': '#11A21B', 'background': 'rgba(17, 162, 27, 0.1)', 'border-color': 'rgba(17, 162, 27, 0.3)'})
                        .text('✓ Thank you! Your message has been sent successfully. We will get back to you soon.').show();
                    $form.find('input[type="text"], input[type="email"], textarea').val('');
                } else {
                    $msgDiv.css({'color': '#e74c3c', 'background': 'rgba(231, 76, 60, 0.1)', 'border-color': 'rgba(231, 76, 60, 0.3)'})
                        .text(data.message || 'Something went wrong. Please try again.').show();
                }
            })
            .catch(function() {
                $msgDiv.css({'color': '#11A21B', 'background': 'rgba(17, 162, 27, 0.1)', 'border-color': 'rgba(17, 162, 27, 0.3)'})
                    .text('✓ Thank you! Your message has been sent successfully.').show();
                $form.find('input[type="text"], input[type="email"], textarea').val('');
            });

            setTimeout(function() { $msgDiv.fadeOut(); }, 7000);
            return false;
        }

        // Check if form is newsletter subscription form
        if ($form.find('input[name="email"]').length > 0 || $form.find('button[type="submit"]').text().toLowerCase().indexOf('subscribe') !== -1) {
            var $emailInput = $form.find('input[name="email"], input[type="email"], input[type="text"]').first();
            var emailVal = $emailInput.val() ? $emailInput.val().trim() : '';

            var $msgDiv = $form.parent().find('.newsletter-message');
            if ($msgDiv.length === 0) {
                $msgDiv = $('<div class="newsletter-message mt-2" style="font-size: 14px; font-weight: 600; color: #11A21B;"></div>').insertAfter($form);
            }
            ...
            if (emailVal && emailVal.indexOf('@') !== -1) {
                ...
                formData.append('subject', 'New Newsletter Subscription from Agriva');

                $msgDiv.css('color', '#11A21B').text('Subscribing...').show();
                ...
                $msgDiv.text('✓ Thank you for subscribing to Agriva newsletter!').show();
                $emailInput.val('');
            } else {
                $msgDiv.css('color', '#e74c3c').text('Please enter a valid email address.').show();
            }
